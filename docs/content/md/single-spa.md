...
## [Overview](https://single-spa.js.org/docs/recommended-setup/#overview)
...
## [Alternatives](https://single-spa.js.org/docs/recommended-setup/#alternatives)
...
## [In-browser versus build-time modules](https://single-spa.js.org/docs/recommended-setup/#in-browser-versus-build-time-modules)
...
The way to tell webpack and rollup to leave some dependencies untouched during the build, so that they come from the browser, is via [webpack externals](https://webpack.js.org/configuration/externals/#root) and [rollup externals](https://rollupjs.org/guide/en/#external).
...
## [Import Maps](https://single-spa.js.org/docs/recommended-setup/#import-maps)
...
[Import Maps](https://github.com/WICG/import-maps) are a browser specification for aliasing "import specifiers" to a URL. An import specifier is the string indicating which module to load. Examples:
...
## [Module Federation](https://single-spa.js.org/docs/recommended-setup/#module-federation)

[Module Federation](https://dev.to/marais/webpack-5-and-module-federation-4j1i) is a webpack-specific technique for sharing [build-time modules](https://single-spa.js.org/docs/recommended-setup/#in-browser-versus-build-time-modules). It involves each microfrontend bundling all of its dependencies, even the shared ones. This means that there are multiple copies of each shared dependency - one per microfrontend. In the browser, the first copy of the shared dependency will be downloaded, but subsequent microfrontends will reuse that shared dependency without downloading their copy of it.
...
## [Lazy loading](https://single-spa.js.org/docs/recommended-setup/#lazy-loading)
...
Lazy loading is when you only download JavaScript code that the user needs for the current page, instead of all JavaScript up front. It is a technique for improving the performance of your application by decreasing the time-to-meaningful-render when you initially load the page. If you use [single-spa loading functions](https://single-spa.js.org/docs/configuration#loading-function-or-application), you already have built-in lazy loading for your applications and parcels. Since an application is an "in-browser module," this means that you are only downloading the in-browser modules in your import map when you need them.
...
## [Local development](https://single-spa.js.org/docs/recommended-setup/#local-development)
...
## [Build tools (Webpack / Rollup)](https://single-spa.js.org/docs/recommended-setup/#build-tools-webpack--rollup)
...
15. For webpack, set [`output.devtoolNamespace`](https://webpack.js.org/configuration/output/#outputdevtoolnamespace) to your MFE's name. This helps namespace your sourcemaps to each MFE.

For a bit more information specific to webpack code splits, see [the code splits FAQ](https://single-spa.js.org/docs/faq#code-splits).

## [Utility modules (styleguide, API, etc)](https://single-spa.js.org/docs/recommended-setup/#utility-modules-styleguide-api-etc)
...
## [Cross microfrontend imports](https://single-spa.js.org/docs/recommended-setup/#cross-microfrontend-imports)
...
To make cross microfrontend imports possible, configure your bundler so that the microfrontends are treated as "externals" ([webpack docs](https://webpack.js.org/configuration/externals/#root) / [rollup docs](https://rollupjs.org/guide/en/#external)). Marking them as externals ensures that they are treated as [in-browser modules](https://single-spa.js.org/docs/recommended-setup/#in-browser-versus-build-time-modules) instead of build-time modules.
...
## [Shared dependencies](https://single-spa.js.org/docs/recommended-setup/#shared-dependencies)
...
### [Comparison of approaches](https://single-spa.js.org/docs/recommended-setup/#comparison-of-approaches)
...
### [Sharing with Module Federation](https://single-spa.js.org/docs/recommended-setup/#sharing-with-module-federation)
...
## [Deployment and Continuous Integration (CI)](https://single-spa.js.org/docs/recommended-setup/#deployment-and-continuous-integration-ci)
...
a) Your CI makes a `curl` HTTP call to a running instance of [import-map-deployer](https://github.com/single-spa/import-map-deployer), which updates the import map in a concurrent-safe way. b) Your CI runner pulls down the import map, modify it, and reuploads it.
...
## [Applications versus parcels versus utility modules](https://single-spa.js.org/docs/recommended-setup/#applications-versus-parcels-versus-utility-modules)

Single-spa has [different categories](https://single-spa.js.org/docs/microfrontends-concept#types-of-microfrontends) of microfrontends. It is up to you where and how you use each of them. However, the single-spa core team recommends the following:
...
2. Move fixed navigation menus into their own single-spa applications. Implement their [activity functions](https://single-spa.js.org/docs/configuration#activity-function) to be active by default, only unmounting for the login page.
...
## [Inter-app communication](https://single-spa.js.org/docs/recommended-setup/#inter-app-communication)
...
### [Functions, components, logic, and environment variables](https://single-spa.js.org/docs/recommended-setup/#functions-components-logic-and-environment-variables)
...
### [API Data](https://single-spa.js.org/docs/recommended-setup/#api-data)
...
### [UI State](https://single-spa.js.org/docs/recommended-setup/#ui-state)
...
## [State management](https://single-spa.js.org/docs/recommended-setup/#state-management)
